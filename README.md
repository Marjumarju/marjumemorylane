...
The photos in [`src/assets`](src/assets) used to be `*.asset.json` pointers whose `url`
...
which Vite hashes into `.output/public/assets`. Each file carries the extension that matches its
actual bytes (`tanel.webp` really is WebP, despite Lovable naming the asset `tanel.png`), so the
static handler sends the right `Content-Type`.
